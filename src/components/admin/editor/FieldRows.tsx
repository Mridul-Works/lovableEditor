...
function Header({ field, edited, dirty, onReset }: Pick<RowProps, "field" | "edited" | "dirty" | "onReset">) {
  return (
    <div className="mb-1.5 flex items-center gap-2 text-xs">
      <span className="rounded bg-slate-100 px-1.5 py-0.5 font-semibold uppercase tracking-wide text-slate-500">
        {TYPE_LABEL[field.type]}
      </span>
      <code className="truncate text-[11px] text-slate-400" title={field.key}>{field.key}</code>
      {dirty ? (
        <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700">unsaved</span>
      ) : edited ? (
        <span className="rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-semibold text-indigo-600">edited</span>
      ) : null}
      {edited || dirty ? (
        <button
          type="button"
          onClick={onReset}
          className="ml-auto text-[11px] text-slate-400 hover:text-slate-700 hover:underline"
    ...
    <div>
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className={`${inputClass} font-mono text-xs ${blocked ? "border-red-400" : ""}`}
          data-field-input
          spellCheck={false}
        />
        {value && !blocked ? (
      ...
      {blocked ? (
        <p className="mt-1 text-xs text-red-600">Only http(s), mailto:, tel:, relative paths and #anchors are allowed.</p>
      ) : null}
    </div>
  );
}
...
function MediaActions({
  ...
  const btn = "rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50";
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <button type="button" onClick={onUrl} className={`${btn} ${urlOpen ? "bg-slate-100" : ""}`}>URL</button>
      {children}
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="flex items-start gap-4">
        <div className="relative h-20 w-32 shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-[linear-gradient(45deg,#f1f5f9_25%,transparent_25%,transparent_75%,#f1f5f9_75%),linear-gradient(45deg,#f1f5f9_25%,#fff_25%,#fff_75%,#f1f5f9_75%)] bg-[length:16px_16px] bg-[position:0_0,8px_8px]">
          {value ? (
          ...
          {placeholder ? (
            <span className="absolute inset-x-0 bottom-0 bg-amber-400/90 px-1 text-center text-[10px] font-semibold text-amber-950">
              placeholder
            </span>
          ) : null}
        </div>
        <div className="min-w-0 flex-1 space-y-2">
          ...
          {upload.error ? <p className="text-xs text-red-600">{upload.error.message}</p> : null}
        </div>
      </div>
    </div>
  );
...
export function VideoRow(props: RowProps) {
  ...
  return (
    <div>
      <Header {...props} />
      <div className="flex items-start gap-4">
        <div className="h-20 w-32 shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-black">
          {value && !blocked ? (
            <video key={value} src={value} muted preload="metadata" className="h-full w-full object-cover" />
          ) : (
            <span className="flex h-full items-center justify-center text-[11px] text-slate-400">No video</span>
          )}
        </div>
        <div className="min-w-0 flex-1 space-y-2">
          <p className="truncate text-xs text-slate-500" title={value}>
